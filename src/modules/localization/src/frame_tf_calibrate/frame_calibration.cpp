...
namespace frame_calibration_node
{
    ...
    void FrameCalibrationNode::initRosComm()
    {
        ...
        odom_sub = frameCalibrationNode_nh.subscribe("/localization/wheel_odom", 50, &FrameCalibrationNode::odomCallback, this);
    }

    void FrameCalibrationNode::odomCallback(const nav_msgs::Odometry& odom_msg)
    {
        float speed_threshold;
        frameCalibrationNode_nh.getParam("/frame_calibration/speed_threshold", speed_threshold);

        if(odom_msg.twist.twist.linear.x >= speed_threshold)
        {
            speed_threshold_met = true;
        }
        else
        {
            speed_threshold_met = false;
        }
    }
