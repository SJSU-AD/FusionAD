...
class PointCloudSegmenter {
  private:
    pcl::PointIndices ground_pts;
    pcl::PointIndices non_ground_pts;
    std::vector<Vec3> p_gnd;  //Pts belonging to ground surface
    std::vector<Vec3> p_ngnd; //Pts not belonging to ground surface

    std::vector<pcl::PointIndices::Ptr> GetXSegments(int seg_size, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud);
  ...
  public:
    ...
    void GroundPlaneFitting( pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud ); //Main Loop

    void ExtractInitialSeeds(pcl::PointIndices::Ptr& cloud_seg, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud, pcl::PointIndices::Ptr& seeds); //Returns inital seeds to be used in first plane model estimation

    pcl::PointXYZ CalculatePlaneNormal(pcl::PointIndices::Ptr& cur_p_gnd, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud); //Returns the normal of the estimated ground plane model

    pcl::PointIndices GetGroundPoints( void ) {  //Returns points that have been classified as ground points
      return ground_pts;
    }
